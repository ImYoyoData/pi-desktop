...
export const TOOL_PRUNE_MINIMUM_TOKENS = 8_000;
/** Match Pi compaction serialization truncation for tool results. */
export const TOOL_PRUNE_MAX_CHARS = 800;
...
export function pruneOldToolResults(
	...
	const minimumReclaim =
		opts?.minimumReclaimTokens ?? TOOL_PRUNE_MINIMUM_TOKENS;
	const maxChars = opts?.maxResultChars ?? TOOL_PRUNE_MAX_CHARS;
	const protectedNames = new Set(
		opts?.protectedToolNames
			? [...opts.protectedToolNames]
	...
	for (let i = messages.length - 1; i >= 0; i--) {
		...
		if (text.length <= maxChars) {
			...
			if (
				...
				msg.content.some((p) => (p as { data?: unknown })?.data)
			) {
