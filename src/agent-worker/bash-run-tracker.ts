...
export function createTrackedBashOperations(
	...
	if (base) {
		const operations: BashOperations = {
			exec: async (command, cwd, options) => {
				...
				// Echo the command into the run output so the Running panel terminal
				// always shows what is being executed — even when the command produces
				// no stdout (e.g. `cd`, quiet installs, long-running servers).
				hooks.onOutput(id, `$ ${spawnCommand}\n`);

				const toolPromise = new Promise<{ exitCode: number | null }>(
					(resolve, reject) => {
	...
	const operations: BashOperations = {
		exec: async (command, cwd, options) => {
			...
			// Echo the command into the run output (same as the base-exec branch).
			hooks.onOutput(id, `$ ${spawnCommand}\n`);
