...
function looksLikeCommand(out: string): boolean {
	return (
		Boolean(out.trim()) &&
		!/could not find|not found|INFO: Could not/i.test(out)
	);
}
...
async function probePiVersion(piPath: string): Promise<string | null> {
	try {
		...
		return (
			m?.[0] ?? (looksLikeCommand(text) ? text.split(/\r?\n/)[0]!.trim() : null)
		);
	} catch {
		return null;
	}
...
async function listAvailableMethods(): Promise<PiCliInstallMethod[]> {
	...
	if (process.platform === "win32") {
		if (
			(await resolveCommand("powershell")) ||
			(await resolveCommand("pwsh"))
		) {
	...
	if (process.platform !== "win32") {
		...
		(await resolveCommand("curl.exe")) ||
		(await resolveCommand("curl"))
	) {
		// Windows curl can still fetch scripts; prefer powershell for official .ps1
		if (!methods.includes("powershell")) methods.push("curl");
	}
	// Last resort for machines with no system Node/npm: the app's bundled npm
	// (Electron's Node + the shipped npm package) can install the pi CLI too.
	if (bundledNpmAvailable()) methods.push("bundled-npm");
	return methods;
}
...
async function installWithBun(): Promise<MethodResult> {
	...
	const r = await runCapture(bun, ["add", "-g", PI_CLI_PACKAGE], {
		timeoutMs: 600_000,
	});
	return {
		ok: r.code === 0,
...
async function installWithPnpm(): Promise<MethodResult> {
	...
	const r = await runCapture(pnpm, ["add", "-g", PI_CLI_PACKAGE], {
		timeoutMs: 600_000,
	});
	return {
		ok: r.code === 0,
...
/** Install the pi CLI via the app's bundled npm (machines without system Node). */
async function installWithBundledNpm(): Promise<MethodResult> {
	const cmd = bundledNpmCommand();
	if (!cmd) {
		return { ok: false, log: "bundled npm unavailable" };
	}
	...
	const r = await runCapture(
		...
		{ timeoutMs: 600_000, env: bundledNpmEnv() },
	);
	return {
		ok: r.code === 0,
		log: `${r.stdout}\n${r.stderr}`.trim() || bundledNpmLabel(),
	};
}
...
async function installWithPowershell(): Promise<MethodResult> {
	...
	const pwsh =
		(await resolveCommand("pwsh")) || (await resolveCommand("powershell"));
		...
		const opened = await new Promise<boolean>((resolve) => {
			...
			try {
				const child = spawn(
					...
					[
						"-e",
						`bash -lc 'curl -fsSL "${PI_INSTALL_SH}" | sh; echo; read -n1 -r -p \"Press any key…\"'`,
					],
...
export async function installPiCli(): Promise<PiCliInstallResult> {
	...
	try {
		broadcastProgress({
			phase: "detect",
			method: null,
			message: "Detecting package managers…",
		});
		...
		if (!methods.length) {
			...
			broadcastProgress({
				...
				method: null,
				message:
					"No installer found (need bun, pnpm, npm, powershell, or curl)",
			});
			return {
				ok: false,
				method: null,
			...
			if (result.ok) {
				...
				if (
					...
					if (!result.openedExternal) {
						...
						lastErr =
							result.log || `${method} did not open an installer window`;
						continue;
					}
					broadcastProgress({
						phase: "done",
				...
				broadcastProgress({
					...
					method,
					message:
						"Install finished — restart the app if `pi` is not found yet",
				});
				return {
					ok: true,
					method,
...
export function registerPiCliIpc(): void {
	...
	ipcMain.handle(IpcChannels.piCli.shouldPrompt, async () =>
		shouldPromptPiCliInstall(),
	);
