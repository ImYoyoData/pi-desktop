 ...
 *  the live session, but the button state survives. */
const persistedBySession = new Map<string, CheckpointSummary[]>();
...
function persistFileFor(sessionId: string): string {
	const safe = sessionId.replace(/[^A-Za-z0-9._-]/g, "_");
	return persistDir ? path.join(persistDir, `${safe}.json`) : "";
}

/** Point the summary store at a directory; load existing summaries from disk. */
export function initCheckpointPersistence(dir: string): void {
	persistDir = dir;
	...
	try {
		...
		for (const ent of entries) {
			...
			try {
				...
				const rows = JSON.parse(raw) as CheckpointSummary[];
				const sessionId = ent.name.slice(0, -5);
				if (Array.isArray(rows)) persistedBySession.set(sessionId, rows);
			} catch {
				// corrupt file — skip
			}
		}
...
function flushPersisted(): void {
	if (!persistDirty || !persistDir) return;
	persistDirty = false;
	try {
		for (const [sessionId, rows] of persistedBySession) {
			if (!rows.length) continue;
			fs.writeFileSync(persistFileFor(sessionId), JSON.stringify(rows), "utf8");
		}
	} catch {
		// ignore persistence failures
	}
...
/** Record a finished summary so the revert button survives reloads. */
function persistSummary(sessionId: string, userMessageId: string): void {
	if (!persistDir) return;
	const cp = byKey.get(keyOf(sessionId, userMessageId));
	const summary = cp ? toSummary(cp) : null;
	if (!summary) return;
	if (summary.status === "capturing") return;
	const rows = persistedBySession.get(sessionId) ?? [];
	const idx = rows.findIndex((r) => r.userMessageId === userMessageId);
	if (idx >= 0) rows[idx] = summary;
	else rows.push(summary);
	// Keep the newest 200 summaries per session to bound the file size.
	if (rows.length > 200) rows.splice(0, rows.length - 200);
	persistedBySession.set(sessionId, rows);
	persistDirty = true;
	flushPersisted();
}

/** Summaries for a session (from disk cache + live memory), for UI restore. */
export function listSessionCheckpointSummaries(
	sessionId: string,
): CheckpointSummary[] {
	const live: CheckpointSummary[] = [];
	for (const cp of byKey.values()) {
		if (cp.sessionId === sessionId && cp.status !== "capturing") {
			live.push(toSummary(cp));
		}
	}
	const persisted = persistedBySession.get(sessionId) ?? [];
	const seen = new Set(live.map((s) => s.userMessageId));
	return [...live, ...persisted.filter((s) => !seen.has(s.userMessageId))];
}
...
export function finishCheckpoint(
	...
	persistSummary(sessionId, userMessageId);
	return toSummary(cp);
}
...
export function finishActiveCheckpoint(
	sessionId: string,
...
export function listReadyCheckpoints(sessionId: string): CheckpointSummary[] {
	...
	for (const cp of byKey.values()) {
		if (cp.sessionId === sessionId && cp.status === "ready")
			out.push(toSummary(cp));
	}
	return out;
}
...
export function revertCheckpoint(
	...
	if (
		...
		!rootsEqual(currentWorkspaceRoot, cp.workspaceRoot)
	) {
		return {
			ok: false,
			restored: 0,
