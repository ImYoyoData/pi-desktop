...
const api = {
	...
	window: {
		...
		maximize: () =>
			ipcRenderer.invoke(IpcChannels.window.maximize) as Promise<void>,
		close: () => ipcRenderer.invoke(IpcChannels.window.close) as Promise<void>,
		forceClose: () =>
			ipcRenderer.invoke(IpcChannels.window.forceClose) as Promise<void>,
		isMaximized: () =>
			ipcRenderer.invoke(IpcChannels.window.isMaximized) as Promise<boolean>,
		setThemeSource: (source: "system" | "light" | "dark") =>
			ipcRenderer.invoke(
				IpcChannels.window.setThemeSource,
				source,
			) as Promise<void>,
	...
	workspace: {
		get: () =>
			ipcRenderer.invoke(IpcChannels.workspace.get) as Promise<string | null>,
		open: () =>
			ipcRenderer.invoke(IpcChannels.workspace.open) as Promise<string | null>,
		pick: () =>
	...
	sessions: {
		list: (cwd: string) =>
			ipcRenderer.invoke(IpcChannels.sessions.list, cwd) as Promise<
				SessionSummary[]
			>,
		create: (cwd: string) =>
			ipcRenderer.invoke(
		...
		tryCommand: (sessionId: string, command: AgentCommand) =>
			ipcRenderer.invoke(
				IpcChannels.sessions.tryCommand,
				sessionId,
				command,
			) as Promise<unknown | undefined>,
		killWorker: (sessionId: string) =>
			ipcRenderer.invoke(
				IpcChannels.sessions.killWorker,
				sessionId,
			) as Promise<void>,
		restartWorker: (sessionId: string) =>
			ipcRenderer.invoke(
				IpcChannels.sessions.restartWorker,
				sessionId,
			) as Promise<void>,
		...
		cacheImage: (sessionId: string, source: { dataUrl?: string; url?: string }) =>
			ipcRenderer.invoke(
				...
				sessionId,
				source,
			) as Promise<{ filePath: string; mimeType: string; dataUrl: string }>,
		...
		status: (sessionId: string, cwd: string) =>
			ipcRenderer.invoke(
				IpcChannels.sessions.status,
				sessionId,
				cwd,
			) as Promise<SessionStatus | null>,
		getInfo: (sessionId: string) =>
			ipcRenderer.invoke(
				IpcChannels.sessions.getInfo,
				sessionId,
			) as Promise<SessionInfoResult>,
		onWorkerReady: (callback: (sessionId: string) => void): (() => void) => {
			const listener = (_e: Electron.IpcRendererEvent, sessionId: string) =>
				callback(sessionId);
	...
	runs: {
		list: (workspaceRoot: string) =>
			ipcRenderer.invoke(IpcChannels.runs.list, workspaceRoot) as Promise<
				AgentRunSnapshot[]
			>,
	...
	fs: {
		watch: (root: string) =>
			ipcRenderer.invoke(IpcChannels.fs.watch, root) as Promise<{
				ok: boolean;
			}>,
		unwatch: () =>
			ipcRenderer.invoke(IpcChannels.fs.unwatch) as Promise<{ ok: boolean }>,
		onChanged: (
			callback: (payload: {
				root: string;
	...
	git: {
		...
		conflictContent: (relativePath: string) =>
			ipcRenderer.invoke(
				IpcChannels.git.conflictContent,
				relativePath,
			) as Promise<GitConflictContentResult>,
	...
	models: {
		get: () =>
			ipcRenderer.invoke(IpcChannels.models.get) as Promise<ModelsGetResult>,
	...
	preview: {
		read: (filePath: string) =>
			ipcRenderer.invoke(
				IpcChannels.preview.read,
				filePath,
			) as Promise<PreviewResult>,
		write: (filePath: string, content: string) =>
			ipcRenderer.invoke(
				IpcChannels.preview.write,
				filePath,
				content,
			) as Promise<void>,
		pickFile: () =>
	...
	browser: {
		startSelect: (webContentsId: number) =>
			ipcRenderer.invoke(
				IpcChannels.browser.startSelect,
				webContentsId,
			) as Promise<{ ok: true } | { ok: false; reason: "csp" | "missing" }>,
		stopSelect: (webContentsId: number) =>
			ipcRenderer.invoke(
				IpcChannels.browser.stopSelect,
				webContentsId,
			) as Promise<void>,
		...
		registerGuest: (pageWebContentsId: number) =>
			ipcRenderer.invoke(
				IpcChannels.browser.registerGuest,
				pageWebContentsId,
			) as Promise<{
				ok: boolean;
			}>,
		reportTab: (info: {
			...
			ipcRenderer.invoke(IpcChannels.browser.reportTab, info) as Promise<{
				ok: boolean;
			}>,
		unreportTab: (tabId: string) =>
			ipcRenderer.invoke(IpcChannels.browser.unreportTab, tabId) as Promise<{
				ok: boolean;
			}>,
		openTabAck: (payload: {
			requestId: string;
			...
			ipcRenderer.invoke(IpcChannels.browser.openTabAck, payload) as Promise<{
				ok: boolean;
			}>,
		...
		onToggleEmbeddedDevTools: (callback: () => void) => {
			...
			return () => {
				ipcRenderer.removeListener(
					IpcChannels.browser.toggleEmbeddedDevTools,
					listener,
				);
			};
		},
	...
	asr: {
		status: () =>
			ipcRenderer.invoke(IpcChannels.asr.status) as Promise<AsrStatus>,
		setEnabled: (enabled: boolean) =>
			ipcRenderer.invoke(
				IpcChannels.asr.setEnabled,
				enabled,
		...
		setDownloadMirror: (mirror: string) =>
			ipcRenderer.invoke(
				IpcChannels.asr.setDownloadMirror,
				mirror,
			) as Promise<AsrStatus>,
		install: () =>
			ipcRenderer.invoke(IpcChannels.asr.install) as Promise<AsrStatus>,
		installFromUrl: (url: string) =>
			ipcRenderer.invoke(
				IpcChannels.asr.installFromUrl,
				url,
			) as Promise<AsrStatus>,
		pickModel: () =>
			ipcRenderer.invoke(IpcChannels.asr.pickModel) as Promise<string | null>,
		importModel: (filePath: string) =>
			ipcRenderer.invoke(
				IpcChannels.asr.importModel,
				filePath,
			) as Promise<AsrStatus>,
		reinstallRuntime: () =>
		...
		importRuntime: (filePath: string) =>
			ipcRenderer.invoke(
				IpcChannels.asr.importRuntime,
				filePath,
			) as Promise<AsrStatus>,
		cancelInstall: () =>
			ipcRenderer.invoke(IpcChannels.asr.cancelInstall) as Promise<{
				ok: boolean;
			}>,
		uninstall: () =>
			ipcRenderer.invoke(IpcChannels.asr.uninstall) as Promise<AsrStatus>,
		transcribe: (pcm: Int16Array, sampleRate: number) =>
			ipcRenderer.invoke(IpcChannels.asr.transcribe, {
				pcm,
				sampleRate,
			}) as Promise<string>,
		streamStart: () =>
		...
		getCloudConfig: () =>
			ipcRenderer.invoke(IpcChannels.asr.getCloudConfig) as Promise<{
		...
		testCloud: () =>
			ipcRenderer.invoke(IpcChannels.asr.testCloud) as Promise<{
				ok: boolean;
				message: string;
			}>,
	...
	tts: {
		status: () =>
			ipcRenderer.invoke(IpcChannels.tts.status) as Promise<TtsStatus>,
		setEnabled: (enabled: boolean) =>
			ipcRenderer.invoke(
				IpcChannels.tts.setEnabled,
				enabled,
			) as Promise<TtsStatus>,
		install: () =>
			ipcRenderer.invoke(IpcChannels.tts.install) as Promise<TtsStatus>,
		uninstall: () =>
	...
	update: {
		...
		openGithub: () =>
			ipcRenderer.invoke(IpcChannels.update.openGithub) as Promise<void>,
		openReleases: () =>
			ipcRenderer.invoke(IpcChannels.update.openReleases) as Promise<void>,
		openAuthorEmail: () =>
			ipcRenderer.invoke(IpcChannels.update.openAuthorEmail) as Promise<void>,
	...
	piCli: {
		status: () =>
			ipcRenderer.invoke(IpcChannels.piCli.status) as Promise<PiCliStatus>,
		shouldPrompt: () =>
		...
		openDocs: () =>
			ipcRenderer.invoke(IpcChannels.piCli.openDocs) as Promise<void>,
		openSite: () =>
			ipcRenderer.invoke(IpcChannels.piCli.openSite) as Promise<void>,
		onProgress: (callback: (progress: PiCliInstallProgress) => void) => {
			const listener = (
				_event: Electron.IpcRendererEvent,
				progress: PiCliInstallProgress,
			) => callback(progress);
	...
	market: {
		list: (opts?: { query?: string; type?: PiPackageType; page?: number }) =>
			ipcRenderer.invoke(
				...
				opts,
			) as Promise<PiPackageListResult>,
		install: (packageName: string) =>
			ipcRenderer.invoke(
				IpcChannels.market.install,
				packageName,
			) as Promise<PiPackageInstallResult>,
	},
	checkpoint: {
		...
		list: (sessionId: string) =>
			ipcRenderer.invoke(IpcChannels.checkpoint.list, sessionId) as Promise<
				{
					sessionId: string;
					userMessageId: string;
					status: "capturing" | "ready" | "reverted" | "empty";
	...
	terminal: {
		...
		getScrollback: (id: string) =>
			ipcRenderer.invoke(IpcChannels.terminal.getScrollback, id) as Promise<
				string | null
			>,
